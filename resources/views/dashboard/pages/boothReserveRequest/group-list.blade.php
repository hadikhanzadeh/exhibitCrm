...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <header class="title">
                <h1> {{ __('Booth Reserve requests for ') . $title }}</h1>
                <a class="btn btn-outline-primary" href="{{ route('dashboard.boothReserve') }}">{{ __('back') }}</a>
            </header>
        </div>
    </div>
                        ...
                        @if(!$items->isEmpty())
                            ...
                            @foreach($items as  $item)
                                <tr>
                                    ...
                                    <td class="action">
                                        <a href="{{ route('dashboard.viewBoothReserve',$item->id) }}"><i
                                                class="icon-eye-2"></i></a>
                                        <a class="delete-item"
                                           href="{{ route('dashboard.destroyBoothReserve',$item->id)  }}"><i
